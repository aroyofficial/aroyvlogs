document.addEventListener('DOMContentLoaded',()=>{
  ...
  const enhanceWalking=text=>text.replace(/(?:Walk\s+)?(?:about\s+)?(\d+(?:\.\d+)?)\s*(km|m)/gi,(_,v,u)=>formatDistance(v,u));
  ...
  document.querySelectorAll('.route-list').forEach(list=>{
    ...
    const p=last.querySelector('.route-card p'); if(!p)return;
    const raw=p.textContent.trim();
    const marker=raw.search(/\bThen\s+/i);
    if(marker>-1){
      const visit=raw.slice(0,marker).trim().replace(/[.]$/,'');
      const transfer=raw.slice(marker).trim().replace(/^Then\s+/i,'');
      p.textContent=visit?visit+'.':'';
      const li=document.createElement('li');li.className='route-transport';li.innerHTML='<strong>Transfer:</strong> '+transfer;last.after(li);
    }
  });

  // Every walking instruction identifies its origin as the previous itinerary location.
  document.querySelectorAll('.route-list').forEach(list=>{
    const steps=[...list.querySelectorAll(':scope > .route-step')];
    let previous='';
    steps.forEach(step=>{
      const card=step.querySelector('.route-card');if(!card)return;
      const title=card.querySelector('h3')?.textContent.trim();
      const p=card.querySelector('p');
      if(p){let text=p.textContent.trim();if(previous&&/\b(?:Walk|🚶|About)\b/i.test(text)&&!/\bfrom\b/i.test(text))text=text.replace(/\.$/,'')+' from '+previous+'.';p.textContent=enhanceWalking(text);}
      if(title)previous=title;
    });
  });
  document.querySelectorAll('.route-transport').forEach(el=>{el.innerHTML=enhanceWalking(el.innerHTML);});

  // Only non-pandal transport locations are linked; pandal names retain their Google Maps buttons.
  const linkLocations=root=>{Object.entries(stationLinks).forEach(([name,url])=>{const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),nodes=[];while(w.nextNode())nodes.push(w.currentNode);nodes.forEach(node=>{if(!node.nodeValue.includes(name)||node.parentElement.closest('a'))return;const parts=node.nodeValue.split(name),frag=document.createDocumentFragment();parts.forEach((part,i)=>{if(i){const a=document.createElement('a');a.href=url;a.target='_blank';a.rel='noopener';a.textContent=name;a.className='route-location-link';frag.appendChild(a);}if(part)frag.appendChild(document.createTextNode(part));});node.parentNode.replaceChild(frag,node);});});};
